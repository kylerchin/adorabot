...
export function inspectInteraction(interaction) {
       ...
       //transforms the user id list into a list to be banned
        ...
        if (arrayOfUserIdsToLookup) {
            
    interaction.reply(`Looking up ${arrayOfUserIdsToLookup.length} users...`)

    
    sendinspectembedsfromuseridlist(interaction,arrayOfUserIdsToLookup)
        } else {
            
    interaction.reply(`No user ids or mentions in command. Please fix your request and try again.`)
        }
}

export function sendinspectembedsfromuseridlist(message,arrayOfUserIdsToLookup) {
...
export function inspect(args:inspectFunction) {
    ...
    logger.discordInfoLogger.info(`Inspect responding to ${args.message.id}`)
        ...
        sendinspectembedsfromuseridlist(args.message,arrayOfUserIdsToLookup)
